    ...
    if (confirm("Are you sure you want to approve this topup?")) {
      this.userService.approveTopUp({ txnId: txnId }).subscribe(
        (response) => {
          ...
          const currentPendingCount = this.lst.filter(
            (transaction: any) => transaction.status === 2
          ).length;
          const updatedPendingCount = currentPendingCount - 1;
          this.recordCreationService.announceUpdate({
            ...
            pendingTransactionCount: updatedPendingCount,
            hasPendingTransactions: updatedPendingCount > 0,
          });
          this.toastrMessageService.showSuccess(
            "Topup approved successfully",
            "Success"
          );
